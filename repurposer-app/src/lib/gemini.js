...
// We extract text via a free CORS proxy
...
export async function generateContentFormats(apiKey, sourceText, context, refImages = []) {
  const genAI = new GoogleGenerativeAI(apiKey);
  // Using gemini-2.5-flash as it supports multimodality.
...
Generate the precise JSON format requested.
  ...
  try {
    ...
    // Parse the JSON output and stringify complex objects back to markdown so ResultsPanel renders them gracefully
    const json = JSON.parse(text);

    // Format ig-post nicely
